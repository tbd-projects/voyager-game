...
menu::MainMenu::MainMenu(graphics::ICanvas &canvas, event_controller::IController &controller, graphics::IGraphicsFactory &factory,
    ...
    auto start_command = std::make_unique<game_manager::command::LevelsMenuCommand>();

    buttons().push_back(
            _button_creator->create(factory, font, std::move(start_command), "Start game")
    );
    auto other_command = std::make_unique<game_manager::command::DoNothing>();
    buttons().push_back(
            _button_creator->create(factory, font, std::move(other_command), "Game info")
    );
